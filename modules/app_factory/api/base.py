...
class API(AppZero):
    ...
    def _locate_api_definition(self, param):
        ...
        if not os.path.exists(self._definition_path):
            raise HTTP(404, "cannot locate API definition '{name}'".format(name=param))
    ...
    def _method_validate(self, method):
        if method not in self.data["supported_methods"]:
            raise HTTP(
                405,
                "'{method}' method is not supported by this API".format(method=method),
            )
    ...
    def get(self, table_id=None, additional_query=None):
        ...
        if table_id:
            if result:
                return result.first()
            raise HTTP(404, "no record found")
    ...
    def post(self, data):
        ...
        if self.data["POST"].get("allowed_fields") and any(
            ...
            raise HTTP(400, "unallowed POST field")

        if not self.data["POST"]["allow_duplicates"] and self._db[
            self.data["table_name"]
        ](**data):
            raise HTTP(400, "duplicate record exists")
    ...
    def put(self, table_id, data):
        ...
        if self.data["PUT"].get("allowed_fields") and any(
            ...
            raise HTTP(400, "unallowed PUT field")
